...
def analyze_frame(frame_bgr: np.ndarray) -> dict:
    """
    Run the full analysis on an OpenCV BGR frame and return JSON-friendly results.
    ...
    # === Gender Detection ===
    try:
        # DeepFace accepts numpy arrays; use RGB
    ...
    results_dict = {
        ...
        "coverage_top": round(coverage_top, 2),
            ...
            if not merged.empty:
                ...
                for row in merged.itertuples(index=False):
                    suggestions.append({
                        ...
                        "count": int(row.count),
                        "avg_ratio": round(row.hair_ratio, 3)
                    })
